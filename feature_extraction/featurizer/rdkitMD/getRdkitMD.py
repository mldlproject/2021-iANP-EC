...
from .descriptors.Chi import *
from .descriptors.EState_VSA import *
from .descriptors.Mol import *
from .descriptors.Lipinski import *
from .descriptors.PEOE_VSA import *
from .descriptors.SMR_VSA import *
from .descriptors.SlogP_VSA import *
from .descriptors.VSA_EState import *
from .descriptors.Fragments import *
...
colnames = ['BalabanJ', 'BertzCT', 
            ...
            'EState_VSA8', 'EState_VSA9', 
            
            'Kappa1', 'Kappa2', 'Kappa3', 'ExactMolWt', 'FractionCSP3', 'HallKierAlpha', 'HeavyAtomCount', 'HeavyAtomMolWt', 'Ipc', 
            
            'LabuteASA', 'MaxAbsEStateIndex', 'MaxAbsPartialCharge', 'MaxEStateIndex', 'MaxPartialCharge', 'MinAbsEStateIndex', 'MinAbsPartialCharge', 
            'MinEStateIndex', 'MinPartialCharge', 
            
            'MolLogP', 'MolMR', 'MolWt', 
            
            'NHOHCount', 'NOCount', 'NumAliphaticCarbocycles', 'NumAliphaticHeterocycles', 'NumAliphaticRings', 'NumAromaticCarbocycles', 
            ...
            'PEOE_VSA1', 'PEOE_VSA10', 'PEOE_VSA11', 'PEOE_VSA12', 'PEOE_VSA13', 'PEOE_VSA14', 'PEOE_VSA2', 'PEOE_VSA3', 'PEOE_VSA4', 'PEOE_VSA5', 'PEOE_VSA6', 
...
def extract_rdkitMD(compounds,  exportCSV=False, outputPath=None, tag=None, decimal=3):
    #----------------------------------
    if exportCSV:
        if outputPath == None:
    ...
    if isinstance(compounds, list):
        compounds = compounds
    #----------------------------------
    rdkitMD_df = pd.DataFrame(columns=colnames)
    #----------------------------------
    ...
    for compound in compounds:
        ...
        if mol != None:
            ...
            Chi = get_Chi(mol)
            #----------------------------------
            EState_VSA = get_EState_VSA(mol)
            #----------------------------------
            Mol = get_Mol(mol)
            #----------------------------------
            Lipinski = get_Lipinski(mol)
            #----------------------------------
            PEOE_VSA = get_PEOE_VSA(mol)
            #----------------------------------
            RingCount = Chem.Lipinski.RingCount(mol)
            RingCount_df = pd.DataFrame([RingCount])
            RingCount_df.columns = ['RingCount']
            #----------------------------------
            SMR_VSA = get_SMR_VSA(mol)
            #----------------------------------
            SlogP_VSA = get_SlogP_VSA(mol)
            #----------------------------------
            VSA_EState = get_VSA_EState(mol)
            #----------------------------------
            fr = get_fragments(mol)
            #----------------------------------
            rdkitMD = pd.concat([Chi, EState_VSA, Mol, Lipinski, PEOE_VSA, RingCount_df, SMR_VSA, SlogP_VSA, VSA_EState, fr], axis=1)
            rdkitMD = round(rdkitMD, decimal)
        else:
            rdkitMD = pd.DataFrame([['na']*196])
            rdkitMD.columns = colnames
        #----------------------------------
        rdkitMD_df = pd.concat([rdkitMD_df, rdkitMD])
    #----------------------------------
    rdkitMD_df = rdkitMD_df.reset_index(drop=True)
    rdkitMD_df = pd.concat([compounds_df, rdkitMD_df], axis=1)
    if exportCSV:
        if tag == None:
            tag_ = '' 
        else:
            tag_ = tag
        filePath = outputPath + "rdkitMD" + tag_ + ".csv"
        if os.path.isdir(outputPath):
            rdkitMD_df.to_csv(filePath, index=False)
        else:
