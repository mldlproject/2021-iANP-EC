...
#============================================================
rdkit_md_train = pd.DataFrame(np.load("../data/singlerun/featurised_data/rdkit_md_train.npy"))
rdkit_md_val   = pd.DataFrame(np.load("../data/singlerun/featurised_data/rdkit_md_val.npy"))
rdkit_md_test  = pd.DataFrame(np.load("../data/singlerun/featurised_data/rdkit_md_test.npy"))
rdkit_md_data  = pd.concat([rdkit_md_train, rdkit_md_val, rdkit_md_test], axis=0).reset_index(drop=True)

label_train = np.load("../data/singlerun/featurised_data/label_train.npy").tolist()
label_val   = np.load("../data/singlerun/featurised_data/label_val.npy").tolist()
label_test  = np.load("../data/singlerun/featurised_data/label_test.npy").tolist()
...
dataset_color = [0]*len(label_train) + [1]*len(label_val) + [2]*len(label_test)
...
plt.legend(handles=plot1.legend_elements()[0], labels=['Training', 'PSO-validation', 'Test'])
# plt.legend(handles=plot1.legend_elements()[0], labels=['PSO-validation', 'Training-validation', 'Test'])
plt.xlabel("PC1")
plt.ylabel("PC2")
