...
for clf in baseline_clf:
    probs  = np.array(pd.read_csv("../../results/pso/singlerun/pred/top4/y_prob_test_{}_seed0.csv".format(clf))["predicted_prob"])
    labels = np.load("../../data/singlerun/featurised_data/label_test.npy")
    result = pd.DataFrame(list(printPerformance(labels, probs, decimal=4)))
    empydf = pd.concat([empydf, result], axis=1)

output_path = "../../results/pso/singlerun/"
if os.path.isdir(output_path) is False:
        os.makedirs(output_path)

empydf.columns = ['metrics'] + baseline_clf
empydf.to_csv("../../results/pso/singlerun/combined_result.csv", index=False)   