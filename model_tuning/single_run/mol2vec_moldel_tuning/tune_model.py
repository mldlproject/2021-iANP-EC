...
#--------------------------------------------------------------------#
#Define parameter
X_data = np.load('../../data/featurised_data/mol2vec_train.npy')
...
start_knn = time.time()
knn_tuning(X_data, y_data, tag="mol2vec", para=knn_para)
end_knn = time.time()
print("KNN Time:{} seconds".format(end_knn-start_knn))


start_svm = time.time()
svm_tuning(X_data, y_data, tag="mol2vec", para=svm_para)
end_svm = time.time()
print("SVM Time:{} seconds".format(end_svm-start_svm))


start_xgb = time.time()
xgb_tuning(X_data, y_data, tag="mol2vec", para=xgb_para)
end_xgb = time.time()
print("XGB Time:{} seconds".format(end_xgb-start_xgb))


start_rf = time.time()
rf_tuning(X_data, y_data, tag="mol2vec", para=rf_para)
end_rf = time.time()
