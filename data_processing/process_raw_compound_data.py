...
# Fullly refine
refined_actives   = full_refine(active_compounds)
refined_inactives = full_refine(inactive_compounds)

# Remove conflicted labels
active_labels   = [1]*len(refined_actives)
inactive_labels = [0]*len(refined_inactives)

refined_compounds = refined_actives + refined_inactives   
...
df.to_csv("../data/refined_data/refined_anticancerNP.csv", index=False)

# Check number of compounds per class
num_active   = len(df[df['class']==1]) #372
num_inactive = len(df[df['class']==0]) #639
